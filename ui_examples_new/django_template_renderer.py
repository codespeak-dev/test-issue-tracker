#!/usr/bin/env python3
"""
Django Template Renderer using the actual Django template engine.
This uses Django's built-in template system for reliable rendering.
"""
    ...
    def __init__(self, template_dir: str):
        ...
        if not settings.configured:
            settings.configure(
                ...
                USE_TZ=True,
                ROOT_URLCONF='ui_examples_new.mock_urls',  # Add URL configuration
                TEMPLATES=[{
...
def main():
    """Render all template states to HTML files using Django template engine."""
    if len(sys.argv) != 2:
        print("Usage: python django_template_renderer.py <output_dir>")
        sys.exit(1)
    ...
    if not state_descriptions_file.exists():
        print(f"Error: State descriptions file not found: {state_descriptions_file}")
        sys.exit(1)
